...
# Firebase is initialized via config.py (imported indirectly by firebase_utils)
...
if email:
    ...
    if not valid and not approved:
        ...
        elif choice == "If you don't have a new contribution, request access from Pim directly :)":
            request_access(email)
            st.info("Access requested. Please wait for approval.")
    ...
    if valid or approved:
        ...
        if submissions:
            ...
            # Sort submissions by submitted_at (oldest first)
            submissions = sorted(
                submissions,
